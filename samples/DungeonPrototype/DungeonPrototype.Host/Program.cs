...
var websocketPort = ReadPort(args, "--ws-port", 5043);
var httpPort = ReadPort(args, "--http-port", 8081);
var repositoryRoot = Directory.GetCurrentDirectory();
var webRoot = Path.Combine(repositoryRoot, "samples", "DungeonPrototype", "web");
var sdkRoot = Path.Combine(repositoryRoot, "clients", "typescript", "dist");

if (!File.Exists(Path.Combine(webRoot, "index.html")))
{
    throw new InvalidOperationException(
        "Run the sample from the PartyGameKit repository root so samples/DungeonPrototype/web can be found.");
}
...
await using var dungeonHost = await DungeonPrototypeHost.StartAsync(
    advertisedHost,
    IPAddress.Any,
    websocketPort);
...
{
    joinDescriptor = JoinDescriptorCodec.SerializeText(dungeonHost.JoinDescriptor),
    roomId = dungeonHost.JoinDescriptor.RoomId.Value,
    joinCode = dungeonHost.JoinDescriptor.JoinCode.Value,
}));

Console.WriteLine("PartyGameKit Dungeon Prototype");
...
Console.WriteLine($"Join payload:  {JoinDescriptorCodec.SerializeText(dungeonHost.JoinDescriptor)}");
Console.WriteLine("Open the player URL in two different browser profiles/phones, then choose characters.");
