...
Dihor.GameKit.Networking `0.2.0-preview.8` supports the direct LAN WebSocket and connection-runtime path from `net10.0-android` applications without requiring the `Microsoft.AspNetCore.App` runtime pack.
...
<ItemGroup>
  ...
  <PackageReference Include="Dihor.GameKit.Networking.Runtime" Version="0.2.0-preview.8" />
</ItemGroup>
```
...
The supported prerelease is `v0.2.0-preview.8`. The GitHub Release contains the individual NuGet packages and `dihor-gamekit-networking-nuget-feed-0.2.0-preview.8.zip`, which includes the complete package set and a ready `NuGet.Config`.
