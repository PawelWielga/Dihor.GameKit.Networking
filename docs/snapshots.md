...
Dihor.GameKit.Networking `0.2` base packages do **not** own a generic game snapshot model.
...
Those snapshots may travel through Dihor.GameKit.Networking as opaque `application.message` data.

## Optional Dihor.GameKit.Networking utility
...
Dihor.GameKit.Networking resume restores communication identity/binding only.

After resume, a consumer may choose to send its newest application snapshot, replay events or reconstruct state another way. Dihor.GameKit.Networking does not impose state restoration semantics.
