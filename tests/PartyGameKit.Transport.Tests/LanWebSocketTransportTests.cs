...
{
    [Fact]
    public async Task DirectLanConnection_ExchangesOpaqueMessages()
    {
        await using var transport = await StartTransportAsync();
        ...
        await transport.SendAsync(opened.ConnectionId, Bytes("server-to-client"), timeout.Token);
        var outbound = await client.ReceiveAsync(timeout.Token);
        Assert.Equal(WebSocketMessageType.Binary, outbound.MessageType);
        Assert.Equal("server-to-client", Text(outbound.Payload));

        await client.SendAsync(Bytes("client-to-server"), timeout.Token);
    ...
    [Fact]
    public async Task ProtocolMismatch_IsRejectedBeforeConnectionIsOpened()
    {
        await using var transport = await StartTransportAsync();
        using var timeout = new CancellationTokenSource(TimeSpan.FromSeconds(5));
        ...
        var close = await client.ReceiveAsync(timeout.Token);
    ...
    [Fact]
    public void LanDescriptor_ContainsOnlyTechnicalConnectionMetadata()
    {
    ...
    private static Task<LanWebSocketTransport> StartTransportAsync() =>
